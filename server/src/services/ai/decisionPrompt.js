...
export function buildSystemPrompt() {
  return [
    ...
    'Every position this platform opens MUST be closed the same day (no overnight/carry positions), so only',
    'call BUY/SELL when the move is likely to play out within the remaining hours of today\'s session.',
    '',
    ...
    'drove the call — timeframe confluence (or lack of it) first, then whichever of RSI/MACD/volume/sector/',
    'Nifty sentiment mattered most. confidence is 0-100 and must reflect actual signal strength and agreement:',
...
export function buildUserContent(symbol, ctx) {
  return [
    ...
    ctx.niftySentiment,
    '',
    'Decision: BUY, SELL, or WAIT? Remember: this must close before today\'s market close, and WAIT is the',
    'correct answer whenever the three timeframes don\'t genuinely agree.',
  ].join('\n');
}
